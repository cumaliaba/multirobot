...
def explorer_node():
    ...
    global motor_command_publisher
    motor_command_publisher = rospy.Publisher('/robot1/cmd_vel', Twist, queue_size = 10)
    
    ## Here we set the function laser_callback to recieve new laser messages when they arrive
    rospy.Subscriber("/robot1/scan", LaserScan, laser_callback, queue_size = 1000)
    
    ## Here we set the function map_callback to recieve new map messages when they arrive from the mapping subsystem
    rospy.Subscriber("/robot1/map", OccupancyGrid, map_callback, queue_size = 1000)
